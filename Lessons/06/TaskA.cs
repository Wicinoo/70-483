...
namespace Lessons._06
{
    ...
    public class TaskA
    {
        ...
        public class MorseCode
        {
            ...
            {
                var morseCodeWithDelimiters = MorseCodeConverter.ToMorseCode(text);
                return new MorseCode(morseCodeWithDelimiters);
            }

            public static implicit operator string(MorseCode morseCode)
            {
                return MorseCodeConverter.FromMorseCode(morseCode._morseCodeWithDelimiters, morseCode._morseDelimiter);
            }
        }

        public static class MorseCodeConverter
        {
            private static readonly Dictionary<char, string> _charToMorse;
            ...
            {
                _charToMorse =
                    JsonConvert.DeserializeObject<Dictionary<char, string>>(MorseCodeConstants.MorseAlphabetAsJson);
                _morseToChar = _charToMorse.ToDictionary(x => x.Value, x => x.Key);
            }

            public static string ToMorseCode(string text, string morseCodeDelimiter = MorseCodeConstants.MorseCodesSeparator)
            {
                return string.Join(
                    morseCodeDelimiter,
                    text.Select(character => _charToMorse[char.ToLower(character)]));
            }

            public static string FromMorseCode(string morseCode, string morseCodeDelimiter = MorseCodeConstants.MorseCodesSeparator)
            {
                return string.Join(string.Empty,
                    morseCode.Split(new[] { morseCodeDelimiter }, StringSplitOptions.RemoveEmptyEntries)
                        .Select(str => _morseToChar[str]));
            }
        }
